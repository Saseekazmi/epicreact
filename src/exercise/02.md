...
- imperative is saying how to do where as declarative is like saying what we
  need.
- For example: Draw the picture is declarative where as take pencil, paper and
  sit down and use scale and draw the picture is imperative approach.
- declarative will still have some sort of imperative coding also

### React.createElement

- It will return an object , which is then rendered to screen through ReactDOM
  package.

Syntax: React.createElement(elementType,elementProps,Children1,...,ChildrenN);

Example:
`React.createElement('div',{id:'name', className:'userDetails'},'Hello','','World!')`

children can also passed as an array in props field, for example, above code can
be re-written as
`React.createElement('div',{id:'name', className:'userDetails', children:['Hello','','World!']})`
...
> you'll want to use the new `React.createRoot` API as demonstrated above.
