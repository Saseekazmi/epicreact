...
### 1)How React Parses JSX.

It will use babel loader under the hood. It preload the JSX and converts it into
javascript code and append it to script tag.

Code which needs to converted will be written inside script tag with
`type='text/babel'`, so that browser will not load the script directly.

Inside JSX props can be expended using spread syntax. When we are using spread
syntax, it will overide if values present.

For example

```
const props ={id:'newId',
             className:'newClassName',
             children:'We can specify div content like this also'
            };
const elem = <div id='oldId' {...props} />;

```

In above example, id props value is replaced with newId value. We can use this
method to specify the default value also.

Note:`default value should be given on left side, spread syntax should be defined on right side`
