...
### Reason why we need to give class as className and style in object format.

- when JS engine parses html and create the DOM, it stores value in className
  instead of class. we can check this by $0.className.Similarly, it stores as
  styles in object format.

- we can use spread operator to expand the props. If we want only specific prop
  value and other values to extracted as it is, we can do as below.
