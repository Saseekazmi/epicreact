...
- React.createElement can also accept function as type and arguments as a
  children

- If we call an function directly, no element/component is created in virtual
  dom with function name. But if we call a function through React.createElement,
  React will intrepreat it as component and new element will be created in
  Virtual dom.

  Eg: React.createElement(message,{children:'Hello World'})

- JSX will be converted to React.createElement through babel. if we are creating
  a functional based component, then function name should be capital. Otherwise,
  babel will convert the component type as string intead of function type.
...
`<displayDiv props='test'/>` will be converted to
`React.createElement('displayDiv',{children:'Hello World'})`.

Note that displayDiv is of String type. so it will not properly render. To
render any function component should be of function type. React will take
function starts with capital as function type.

### Reason why react component should render single component.

When JSX component is translated by babel, if we are single element, it will
converted as follows
`React.createElement('compName as function type',props,...childrens)`

But if we are having multiple elements, we cannot combine it to as React
element.

To avoid this we can use react fragments.

### proptypes

- proptypes for typechecking has to be imported before calling component

- ComponentName.propTypes ={ propName to be checked(props, propName ): required
  type }

- it will take some time to render, so it will not work in production mode as it
  is an time consuming process.(check type script as an alternative option)
